...
//    protected $_nameUserRoles = 'tusers_to_roles';
    ...
    static function populate($params) {
        ...
        if (isset($params['apepat']))
            $data['apepat'] = $params['apepat'];
        if (isset($params['apemat']))
            $data['apemat'] = $params['apemat'];
        ...
        if (isset($params['privilegio']))
            $data['privilegio'] = $params['privilegio'];
        if (isset($params['lastlogin']))
            $data['lastlogin'] = $params['lastlogin'];
        if (isset($params['lastpasschange']))
            $data['lastpasschange'] = $params['lastpasschange'];
...
//    }
    ...
    public function columnDisplay() { 
        return array("CONCAT(tusers.name, ' ',tusers.apepat, ' ', tusers.apemat)","CONCAT(troles.name, '')", "IF(tusers.state=1, 'Activo', 'Inactivo')");
    }
